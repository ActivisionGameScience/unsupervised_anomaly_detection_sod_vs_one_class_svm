...
df = pd.read_csv('mouse.csv', header=None, index_col=False, sep=' ', comment='#')

# group by the class column (we will treat the b's as outliers)
# also, drop the class column
grouped = df.groupby(2)
outliers = grouped.get_group('Noise')
outliers = outliers.drop(2,axis=1)
others = df.drop(grouped.get_group('Noise').index)
others = others.drop(2,axis=1)
...
resampled.to_csv('mouse_for_elki.csv', header=None, index=None)
