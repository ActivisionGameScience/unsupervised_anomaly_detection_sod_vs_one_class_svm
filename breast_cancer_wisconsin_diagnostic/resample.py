...
df = pd.read_csv('wdbc.data', header=None, index_col=False)

# drop first column (an id)
df = df.drop(0,axis=1)

# group by the class column (we will treat groups M as outliers)
# also, drop the class column
grouped = df.groupby(1)
outliers = grouped.get_group('M')
outliers = outliers.drop(1,axis=1)
others = df.drop(grouped.get_group('M').index)
others = others.drop(1,axis=1)

# downsample the outliers to 2.72% of the size of the rest
outliers_sample = outliers.sample(n=int(0.0272*others.shape[0]))
...
resampled.to_csv('wdbc_for_elki.csv', header=None, index=None)
